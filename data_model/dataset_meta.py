...
class DigitsDataset(data.Dataset):
    def __init__(
        ...
        logger.info(
            "train=%s/%s val=%s/%s test=%s/%s split=%s",
            train_data.shape,
            train_label.shape,
            val_data.shape,
            val_label.shape,
    ...
    def split_data(self, data, label, split_ratio=0.8, split_ratio_test=0.1, train_val='train', data_name='',index_save_path='data', index_save_name='train_val.npy'):
        ...
        if not os.path.exists(os.path.join(index_save_path, index_save_name_train)):
            logger.info("save split indices: %s %s %s", index_save_name_train, index_save_name_val, index_save_name_test)
            ...
            logger.info(
                "load split indices: %s %s %s from %s",
                index_save_name_train,
                index_save_name_val,
                index_save_name_test,
                os.path.join(index_save_path, index_save_name_train),
            )
    ...
    def cal_near_index(self, k=10, device="cuda", uselabel=False, pca_dim=100, train_val='train', split_ratio=0.8):
        ...
        if not os.path.exists(filename):
            ...
            logger.info("save neighbor index to %s", filename)
        else:
            logger.info("load neighbor index from %s", filename)
...
class DigitsSEQDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.train_val == 'train':
            ...
            logger.info("test split requested")
            data_input_item = self.data[index].astype(np.float32)
            label = self.label[index]
            neighbor_index_list = self.neighbors_index[index]
