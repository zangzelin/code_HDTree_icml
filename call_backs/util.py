...
class TreeNode:
    ...
    def print_tree(self, depth=0):
        ...
        if self.decoder:
            logger.info("leaf node, number of samples: %s", self.samples.shape[0])
        else:
            logger.info(
                "router node: %s left: %s right: %s",
                self.router,
                self.left.name if self.left else None,
                self.right.name if self.right else None,
            )
            if self.left:
    ...
    def output_label_list(self):
        ...
        for i, l in enumerate(reversed(self.list_all_leaf())):
            logger.debug("leaf %s has %s samples", l.name, l.samples.shape[0])
