...
def Plot_subfig_1_1(
    ...
    logger.debug("label color set: %s", color_set)
    fig_list = []
    for c in color_set:
        mask = label == c
...
def Plot_subfig_2_1(global_importance, fea_list):
    ...
    logger.debug("top feature importances: %s", list_xx)
    logger.debug("top feature names: %s", list_yy)
...
def Plot_case_study(
    ...
    if pix > 0:
        ...
        logger.debug("pseudo labels: %s", label_pesodu)
        for j in range(10):
...
def rotate_fea(fea, ins, mask):
    ...
    logger.debug("best rotation index: %s", best_i)
...
def show_local_expl(
    ...
    logger.debug("finished local explanation")
