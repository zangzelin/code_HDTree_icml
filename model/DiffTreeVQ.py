...
class TransformerEncoder(nn.Module):
    ...
    def __init__(
        ...
        if num_input_dim == 3072:
            ...
            nn_type = "nn"
            logger.info("Using fully connected network")
...
class DMTEVT_model(LightningModule):
        ...
        **kwargs,
        ...
        if ckpt_path is not None:
            ...
            if "module." in list(state_dict.keys())[0]:
                logger.info("Loading checkpoint from multi-gpu state dict: %s", ckpt_path)
    ...
    def update_training_str(self, epoch):
        ...
        logger.info("training stage=%s epoch=%s", self.training_str, epoch)
    ...
    def align_the_node_embedding(self, emb):
        ...
        logger.info("Reinitialized all embeddings in tree_node_embedding.")

    def update_node_embedding(self, emb, device):
        logger.info("Update the node embedding")
